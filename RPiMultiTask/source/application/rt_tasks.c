...
void InitRTTasks(uint16_T period[NUM_TASKS])
{
	...
	for (i = 1; i < NUM_TASKS; i++)
	{
		...
		task[i].clock.clockTickL = 0ul;
		task[i].clock.clockTickH = 0ul;
	}
...
*  -------------------------------------------------------  *
*/
void (*pTaskFcn[NUM_TASKS])(void) = {NULL,
									 ExecTask1,
									 ExecTask2,
									 ExecTask3 };
...
void *TaskRoutine(void *arg)
{
	while (1)
	{
		sem_wait(&task[(int)arg].sem);        /* sem_val = 1 */
		pTaskFcn[(int)arg]();
		sem_wait(&task[(int)arg].sem);        /* sem_val = 0 */
	}
} // END: Task1()
